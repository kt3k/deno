...
#[test]
#[cfg(unix)]
async fn deno_run_serve_with_unix_replace_env_addr() {
  ...
  // Create an existing socket file to simulate a stale socket
  // Use std::os::unix::net::UnixListener which doesn't remove the socket on drop
  {
    let _listener = StdUnixListener::bind(&sock).unwrap();
    // Listener is dropped here but the socket file remains
  }
  assert!(sock.exists());
  ...
  let mut child = util::deno_cmd()
    ...
    .env(
      "DENO_SERVE_ADDRESS",
      format!("unix-replace:{}", sock.display()),
    )
    .stderr_piped()
    .spawn()
    .unwrap();
